...
static void run_task(char **argv) {
  ...
  if (thread_tests) {
    ...
    /*
     * process_create_initd(task) 라는 함수를 실행시키고, 이때 인자로 받는
     * task는 command line에서 run 뒤에 썼던 문자열 전체이다. 만약 pintos
     * --fs-disk=10 -p tests/userprog/args-single:args-single -- -q -f run
     * 'args-single onearg' 명령어를 사용했다면, task는 args-single onearg 가
     * 된다.
     */
...
/*
   ARGV[]에 지정된 모든 동작을 null 포인터 센티널(null pointer sentinel)까지
   실행한다. null pointer sentinal -> 배열의 끝을 나타내는 특별한 표시 (널
   포인터) Executes all of the actions specified in ARGV[] up to the null
   pointer sentinel. */
static void run_actions(char **argv) {
  /* An action. */
  struct action {
